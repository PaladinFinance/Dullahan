...
for i in range(0, len(pod_list)):
    ...
    pod_debt_wei = debtGHO.functions.balanceOf(pod_list[i]).call()
    stkAave_balance_wei = stkAAVE.functions.balanceOf(pod_list[i]).call()
    ...
    if(stkAave_balance > actual_needed_stkAave):
        ...
        """
        tx_dict = manager.functions.freeStkAave(pod_list[i]).buildTransaction({
