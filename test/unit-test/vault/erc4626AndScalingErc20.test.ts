...
describe('DullahanVault contract tests - ERC4626 & Scaling ERC20 functions', () => {
    ...
    before(async () => {
        ...
        [admin, reserveManager, votingManager, proposalManager, podManager, fakePod, depositor1, depositor2, depositor3] = await ethers.getSigners();
    ...
    describe('totalAssets', async () => {
        ...
        const user2_withdraw = ethers.utils.parseEther('50')
        ...
        it(' should return the correct amount of funds deposited (+ total Supply to track that amount)', async () => {

            let vault_balance = await stkAave.balanceOf(vault.address)
            let vault_reserve = await vault.reserveAmount()
            expect(await vault.totalAssets()).to.be.eq(vault_balance.sub(vault_reserve))
            expect(await vault.totalSupply()).to.be.eq(vault_balance.sub(vault_reserve))

            await vault.connect(depositor1).deposit(user1_deposit, depositor1.address)

            vault_balance = await stkAave.balanceOf(vault.address)
            vault_reserve = await vault.reserveAmount()
            expect(await vault.totalAssets()).to.be.eq(vault_balance.sub(vault_reserve))
            expect(await vault.totalSupply()).to.be.eq(vault_balance.sub(vault_reserve))
            ...
            await vault.connect(depositor2).withdraw(user2_withdraw, depositor2.address, depositor2.address)
    ...
    describe('convertToShares & convertToAssets', async () => {

        it(' should return the correct values', async () => {

            expect(await vault.convertToShares(0)).to.be.eq(0)
            expect(await vault.convertToShares(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.convertToShares(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.convertToShares(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))

            expect(await vault.convertToAssets(0)).to.be.eq(0)
            expect(await vault.convertToAssets(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.convertToAssets(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.convertToAssets(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))

        });

    });

    describe('previewDeposit & previewMint', async () => {

        it(' should return the correct values', async () => {

            expect(await vault.previewDeposit(0)).to.be.eq(0)
            expect(await vault.previewDeposit(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.previewDeposit(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.previewDeposit(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))

            expect(await vault.previewMint(0)).to.be.eq(0)
            expect(await vault.previewMint(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.previewMint(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.previewMint(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))

        });

    });

    describe('previewWithdraw & previewRedeem', async () => {

        it(' should return the correct values', async () => {

            expect(await vault.previewWithdraw(0)).to.be.eq(0)
            expect(await vault.previewWithdraw(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.previewWithdraw(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.previewWithdraw(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))

            expect(await vault.previewRedeem(0)).to.be.eq(0)
            expect(await vault.previewRedeem(ethers.utils.parseEther('150'))).to.be.eq(ethers.utils.parseEther('150'))
            expect(await vault.previewRedeem(ethers.utils.parseEther('2500'))).to.be.eq(ethers.utils.parseEther('2500'))
            expect(await vault.previewRedeem(ethers.utils.parseEther('558500'))).to.be.eq(ethers.utils.parseEther('558500'))
    ...
    describe('maxDeposit', async () => {
        ...
        it(' should return the correct amount', async () => {
            ...
            expect(await vault.maxDeposit(depositor1.address)).to.be.eq(ethers.constants.MaxUint256)

            expect(await vault.maxDeposit(depositor2.address)).to.be.eq(ethers.constants.MaxUint256)

            expect(await vault.maxDeposit(depositor3.address)).to.be.eq(ethers.constants.MaxUint256)
    ...
    describe('maxMint', async () => {
        ...
        it(' should return the correct amounts', async () => {
            ...
            expect(await vault.maxMint(depositor1.address)).to.be.eq(ethers.constants.MaxUint256)

            expect(await vault.maxMint(depositor2.address)).to.be.eq(ethers.constants.MaxUint256)

            expect(await vault.maxMint(depositor3.address)).to.be.eq(ethers.constants.MaxUint256)

        });

    });

    describe('maxWithdraw & maxRedeem', async () => {
        ...
        beforeEach(async () => {
            ...
            await stkAave.connect(admin).transfer(depositor1.address, user1_deposit.mul(2))
            await stkAave.connect(admin).transfer(depositor2.address, user2_deposit)
            await stkAave.connect(admin).transfer(depositor3.address, user3_deposit)
        ...
        it(' should return the correct values', async () => {
            ...
            expect(await vault.maxWithdraw(depositor1.address)).to.be.eq(0)
            expect(await vault.maxRedeem(depositor1.address)).to.be.eq(0)

            await vault.connect(depositor1).deposit(user1_deposit, depositor1.address)

            expect(await vault.maxWithdraw(depositor1.address)).to.be.eq(await vault.balanceOf(depositor1.address))
            expect(await vault.maxRedeem(depositor1.address)).to.be.eq(await vault.balanceOf(depositor1.address))

            expect(await vault.maxWithdraw(depositor2.address)).to.be.eq(0)
            expect(await vault.maxRedeem(depositor2.address)).to.be.eq(0)

            await vault.connect(depositor2).deposit(user2_deposit, depositor2.address)

            expect(await vault.maxWithdraw(depositor2.address)).to.be.eq(await vault.balanceOf(depositor2.address))
            expect(await vault.maxRedeem(depositor2.address)).to.be.eq(await vault.balanceOf(depositor2.address))
